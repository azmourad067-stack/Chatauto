...
from adaptive_tickets_v52 import select_adaptive_tickets, V52_CONFIG

# =============================================================================
# CONFIG
# =============================================================================

APP_NAME = "HorseProno Quinté V5.2 — Adaptive Tickets"
...
st.title("🏇 HorseProno Quinté V5.2 — Adaptive Tickets")
st.caption(
...
try:
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        # Quinté V5.2 : 70 tickets candidats, puis budget adaptatif 30 / 50 / 70.
        st.subheader("🏇 V5.2 — Quinté adaptatif 30 / 50 / 70")
        q70, q70_exposure, q70_scenarios = generate_quinte_70(ranked)
        if len(q70) == 70:
            q_adapt, adaptive = select_adaptive_tickets(q70, ranked)
            ...
            a1.metric("Profil V5.2", adaptive["profile"].replace("_", " "))
            a2.metric("Tickets conseillés", str(adaptive["ticket_count"]))
            a3.metric("Divergence", f"{100*adaptive['divergence']:.0f} %")
            a4.metric("Mise pleine à 2 €", f"{adaptive['full_stake_eur']:.0f} €")
            st.info(
                f"**{adaptive['label']}** — {adaptive['reason']} "
                f"Consensus : {adaptive['consensus_count']}/7 · confiance : {100*adaptive['confidence']:.0f} %."
            )
            st.caption(
                "Le moteur construit toujours 70 candidats parmi les 252 combinaisons du pool de 10, puis V5.2 retient automatiquement les 30, 50 ou 70 meilleurs. "
                "Règle gelée : Core 30 si consensus ≥6, divergence <30 % et confiance ≥58 % ; couverture 70 seulement si consensus ≤2, divergence ≥50 % ou confiance ≤35 % ; sinon Extension 50. "
                "La mise affichée est une simple référence à 2 € par combinaison, pas une recommandation financière."
            )

            st.markdown(f"**🎯 Portefeuille conseillé : {len(q_adapt)} combinaisons**")
            st.dataframe(q_adapt, use_container_width=True, hide_index=True)
            st.code("\n".join(f"{int(r['Ticket adaptatif']):02d}. [{r['Scénario']}] {r['Combinaison']}" for _, r in q_adapt.iterrows()), language=None)

            with st.expander("Voir la couverture complète des 70 tickets"):
            ...
            st.warning("Il faut au moins 10 partants classés pour générer le portefeuille Quinté adaptatif.")
